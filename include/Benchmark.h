...
/**
 * @brief Benchmark class with functions to evaluate the estimated pose of
 * different datasets
 * @detail Benchmark from Arcuo class should be in "benchmark" folder and
 * linemod benchmark in "benchmarkLINEMOD" folder
 */
class Benchmark {
 public:
  /**
   ...
   * @detail Has to be called every time the detector is run on an image to get
   * stats
   *
   */
  ...
  /**
   * @brief Calculate the error with the BOP 6D benchmark evalutation method by
   * Hodan et al
   *
  ...
  /**
   * @brief Calculate the error with the linemod evalutation method for ambigous
   * objects
   *
   * @param in_estimate Estimated pose
   * @return float error
