...
class OpenGLRender {
 public:
  ...
  /**
   * @brief Read 8bit color image from front buffer to the CPU RAM and convert
   * to opencv mat
   *
   * @return cv::Mat
   */
  ...
  /**
   * @brief Read 16bit depth image from front buffer to the CPU RAM and convert
   * to opencv mat
   *
   * @return cv::Mat
   */
   ...
   */
  void renderColorToFrontBuff(uint16_t in_modelIndice, glm::mat4 in_rotMat,
                              glm::vec3 in_traVec);
   ...
   * @param in_traVec Translation vector of object
   */
  void renderDepthToFrontBuff(uint16_t in_modelIndice, glm::mat4 in_rotMat,
                              glm::vec3 in_traVec);
 ...
 private:
  ...
  /**
   * @brief Function to calculate the view matrix depending on the camera
   * position
   *
   * @param in_vec
   */
