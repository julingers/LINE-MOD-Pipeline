...
/**
 * @brief The class deals with the ICP pose refinement and picks the best pose
 * out of multiple refined poses
 *
 */
class HighLevelLinemodIcp {
 public:
   ...
   * @param in_tolerance The sought after matching score. Changing this is not
   * advised
   * @param in_rejectionScale Rejection Scale of outlier points. Also not
   * advised to change it
   * @param in_numIterations A quality setting for the icp algorithm. Does not
   * seem to have any effect on the outcome.
   * @param in_sampleStep Factor that reduces the amont of points used for ICP.
   * @param in_modelFiles Vector of model file names. It is equal to the class
   * names of the detector
   * @param in_modFolder Location of the model files
   */
  HighLevelLinemodIcp(uint16_t in_iteration, float in_tolerance,
                      float in_rejectionScale, uint16_t in_numIterations,
                      uint16_t in_sampleStep,
   ...
   */
  void prepareDepthForIcp(cv::Mat& in_depth, const cv::Mat& in_camMatrix,
                          cv::Rect& bb);
 ...
 private:
  /**
   * @brief Sets the threshold for the mean difference between the rendered pose
   * and the depth image
   * @detail Lowering the value means that the pose needs to fit better to the
   * depth image
   *
   */
   ...
   */
  void removeIfTooFarFromMean(const cv::Mat& in_mat, cv::Mat& in_res,
                              bool in_removeRows = true);
   ...
   * @param in_threshold Threshold at what value a pixel will become white or
   * 255
   */
  void depthToBinary(cv::Mat& in_gray, cv::Mat& in_binary,
                     uint32_t in_threshold = 1);
