...
void filesInDirectory(std::vector<std::string>& in_filePathVector,
  ...
  if (in_filePathVector.empty()) {
    std::cout << "No " << in_extension << " files found in " << in_path
              << std::endl;
  } else {
    std::cout << "Found " << in_filePathVector.size() << " " << in_extension
              << " files in " << in_path << std::endl;
  }
}
...
void readSettings(CameraParameters& in_camParams,
  ...
  in_camParams.cameraMatrix =
      (cv::Mat1d(3, 3) << in_camParams.fx, 0, in_camParams.cx, 0,
  ...
  fs["distance to match to be considered same object"] >>
      in_tempGenSettings.radiusThresholdNewObject;
  fs["ratio to determine if group is too small"] >>
      in_tempGenSettings.discardGroupRatio;
