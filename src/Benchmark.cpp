...
float Benchmark::calculateErrorHodan(cv::Mat& in_depthImg,
                                     OpenGLRender* in_opengl,
  ...
  cv::threshold(renderedAbsDiff, renderedAbsDiff, errorThreshold, 65536,
                cv::THRESH_BINARY_INV);
  cv::bitwise_and(visibilityIntersection, renderedAbsDiff, absDiffMaskApplied);
  float error = 1 - (float)cv::countNonZero(absDiffMaskApplied) /
                        (float)cv::countNonZero(visibilityCombination);
  if (error < 0.3f) {
    hodanCounter++;
  }
  return error;
...
#pragma omp parallel for
  for (int32_t i = 0; i < model.numVertices; i++) {
    cv::Matx31f vertModel(model.vertices[i].x, model.vertices[i].y,
                          model.vertices[i].z);

    cv::Mat groundTruthRotatedTranslated =
        rotMatGroundTruth * vertModel + cv::Matx31f(groundTruth.translation.x,
                                                    groundTruth.translation.y,
                                                    groundTruth.translation.z);

    cv::Mat estimateTruthRotatedTranslated =
        rotMatEstimate * vertModel + cv::Matx31f(in_estimate.translation.x,
                                                 in_estimate.translation.y,
                                                 in_estimate.translation.z);

    cv::Mat differenceBetweenMat =
        groundTruthRotatedTranslated - estimateTruthRotatedTranslated;
...
void Benchmark::loadModel(OpenGLRender* in_opengl,
                          std::string in_modelLocation) {
...
#pragma omp parallel for
  for (int32_t i = 0; i < subsampledModel.numVertices; i++) {
    cv::Matx31f vertModelGT(subsampledModel.vertices[i].x,
                            subsampledModel.vertices[i].y,
                            subsampledModel.vertices[i].z);

    cv::Mat groundTruthRotatedTranslated =
        rotMatGroundTruth * vertModelGT +
        cv::Matx31f(groundTruth.translation.x, groundTruth.translation.y,
                    groundTruth.translation.z);
    float absDifference = 999999;
    for (int32_t j = 0; j < subsampledModel.numVertices; j++) {
      cv::Matx31f vertModelEst(subsampledModel.vertices[j].x,
                               subsampledModel.vertices[j].y,
                               subsampledModel.vertices[j].z);

      cv::Mat estimateTruthRotatedTranslated =
          rotMatEstimate * vertModelEst +
          cv::Matx31f(in_estimate.translation.x, in_estimate.translation.y,
                      in_estimate.translation.z);
      cv::Mat differenceBetweenMat =
          groundTruthRotatedTranslated - estimateTruthRotatedTranslated;
      cv::Vec3f differenceBetweenVec(differenceBetweenMat.at<float>(0, 0),
...
void Benchmark::calculateVisibilityMasks() {
  ...
  cv::threshold(groundTruthVisibility, groundTruthVisibility,
                visibilityThreshold, 65536, cv::THRESH_BINARY);
  cv::threshold(groundTruthDepthRender, binaryGTRender, 1, 65536,
                cv::THRESH_BINARY);
  groundTruthVisibility = binaryGTRender - groundTruthVisibility;

  estimateVisibility = estimateDepthRender - inputDepth;
  cv::threshold(estimateVisibility, estimateVisibility, visibilityThreshold,
                65536, cv::THRESH_BINARY);
  cv::threshold(estimateDepthRender, binaryEstRender, 1, 65536,
                cv::THRESH_BINARY);
  ...
  cv::bitwise_and(groundTruthVisibility, estimateVisibility,
                  visibilityIntersection);
  cv::bitwise_or(groundTruthVisibility, estimateVisibility,
                 visibilityCombination);
}

cv::Mat Benchmark::renderPose(OpenGLRender* in_opengl,
                              ObjectPose const& in_pose,
                              uint16_t const& in_modelIndice) {
  glm::mat4 viewMat = calculateViewMat(in_pose);
  in_opengl->renderDepthToFrontBuff(in_modelIndice, viewMat,
                                    in_pose.translation);
...
void Benchmark::readGroundTruthPose() {
  std::string filename =
      "benchmark/pose" + std::to_string(imageCounter) + ".yml";
  ...
  groundTruth = {glm::vec3(transVec[0], transVec[1], transVec[2]),
                 glm::toQuat(glmRotMat)};
}
...
void Benchmark::readGroundTruthLinemodDataset() {
  ...
  std::ifstream fileStreamTranslation("benchmarkLINEMOD/tra" +
                                      std::to_string(imageCounter) + ".tra");
  if (!fileStreamTranslation.is_open()) {
    std::cout
        << "Error Opening Ground Truth Translation file from Linemod Dataset"
        << std::endl;
  }
  ...
  std::ifstream fileStreamRotation("benchmarkLINEMOD/rot" +
                                   std::to_string(imageCounter) + ".rot");
  if (!fileStreamRotation.is_open()) {
    std::cout << "Error Opening Ground Truth Rotation file from Linemod Dataset"
              << std::endl;
  }
  int cnt = 0;
...
void Benchmark::printHodanScore() {
  std::cout << "Hodan Score: "
            << (float)hodanCounter * 100 / (float)imageCounter
            << " Counter: " << imageCounter << "            " << std::endl;
}

void Benchmark::printLMScore() {
  std::cout << "LM Score: " << (float)lineCounter * 100 / (float)imageCounter
            << " Counter: " << imageCounter << "            " << std::endl;
}

float Benchmark::length(cv::Vec3f& in_vecA) {
  return sqrt(in_vecA[0] * in_vecA[0] + in_vecA[1] * in_vecA[1] +
              in_vecA[2] * in_vecA[2]);
}
