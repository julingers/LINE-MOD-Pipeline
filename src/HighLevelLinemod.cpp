...
HighLevelLineMOD::HighLevelLineMOD(
    ...
    : onlyColorModality(in_templateSettings.onlyUseColorModality),
      ...
      fx(in_camParams.fx),
      fy(in_camParams.fy),
      fieldOfViewHeight(360.0f / CV_PI *
                        atanf(videoHeight / (2 * in_camParams.fy))),
...
HighLevelLineMOD::~HighLevelLineMOD() { detector.release(); }

std::vector<cv::String> HighLevelLineMOD::getClassIds() {
  return detector->classIds();
}

uint16_t HighLevelLineMOD::getNumClasses() { return detector->numClasses(); }
...
bool HighLevelLineMOD::addTemplate(std::vector<cv::Mat>& in_images,
  ...
  for (size_t q = 0; q < inPlaneRotationMat.size(); q++) {
    cv::warpAffine(mask, maskRotated, inPlaneRotationMat[q],
                   maskRotated.size());
    cv::warpAffine(colorToBinary, colorRotated, inPlaneRotationMat[q],
                   colorToBinary.size());
    cv::warpAffine(in_images[1], depthRotated, inPlaneRotationMat[q],
                   in_images[1].size());
    ...
    cv::Rect boundingBox;
    uint64_t template_id = detector->addTemplate(templateImgs, in_modelName,
...
void HighLevelLineMOD::templateMask(cv::linemod::Match const& in_match,
  ...
  const std::vector<cv::linemod::Template>& templates =
      detector->getTemplates(in_match.class_id, in_match.template_id);
...
bool HighLevelLineMOD::detectTemplate(std::vector<cv::Mat>& in_imgs,
  ...
  const std::vector<std::string> currentClass(
      1, detector->classIds()[in_classNumber]);
  ...
  if (!matches.empty()) {
    ...
    for (auto& potentialMatch : potentialMatches) {
      groupedMatches =
          elementsFromListOfIndices(matches, potentialMatch.matchIndices);
...
void HighLevelLineMOD::groupSimilarMatches() {
  ...
  for (size_t i = 0; i < matches.size(); i++) {
    ...
    for (size_t q = 0; q < numCurrentGroups; q++) {
      if (cv::norm(matchPosition - potentialMatches[q].position) <
...
void HighLevelLineMOD::discardSmallMatchGroups() {
  ...
  for (size_t j = 0; j < numMatchGroups; j++) {
    float ratioToBiggestGroup =
        potentialMatches[j].matchIndices.size() * 100 / biggestGroup;
...
void HighLevelLineMOD::generateRotMatForInplaneRotation() {
  for (int16_t angle = lowerAngleStop; angle <= upperAngleStop;
...
void HighLevelLineMOD::calculateTemplatePose(glm::vec3& in_translation,
  ...
  glm::vec3 camUp =
      normalize(cross(in_cameraPosition, cross(in_cameraPosition, up)));
...
bool HighLevelLineMOD::depthCheck(cv::Mat& in_depth, uint32_t& in_numMatch) {
  if (useDepthImprovement) {
    cv::Rect bb(
        groupedMatches[in_numMatch].x, groupedMatches[in_numMatch].y,
    ...
    int32_t depthDiff =
        ...
        (int32_t)templates[groupedMatches[in_numMatch].template_id]
            .medianDepth -
        depthOffset;
    tempDepth =
        templates[groupedMatches[in_numMatch].template_id].translation.z +
        depthDiff;
...
void HighLevelLineMOD::calcRotation(uint32_t const& in_numMatch,
  ...
  in_quats = toQuat(
      adjustRotation *
      toMat4(templates[groupedMatches[in_numMatch].template_id].quaternions));
}

void HighLevelLineMOD::matchToPixelCoord(uint32_t const& in_numMatch,
                                         float& in_x, float& in_y) {
  in_x = (groupedMatches[in_numMatch].x + videoWidth / 2 -
          templates[groupedMatches[in_numMatch].template_id].boundingBox.x);
  in_y = (groupedMatches[in_numMatch].y + videoHeight / 2 -
          templates[groupedMatches[in_numMatch].template_id].boundingBox.y);
}
...
void HighLevelLineMOD::readColorRanges() {
  ...
  for (uint16_t i = 0; i < detector->numClasses(); i++) {
    ...
    std::string filename = modelFolder +
                           modelFiles[i].substr(0, modelFiles[i].size() - 4) +
                           ".yml";
    cv::FileStorage fs(filename, cv::FileStorage::READ);
    cv::Vec3b tempVec;
