...
Kinect2::Kinect2() {
  ...
  // pipeline = new libfreenect2::CpuPacketPipeline(); //Necessary to prevent
  // issues with OpenGL but slows down alot
  if (pipeline) {
...
void Kinect2::getKinectFrames(cv::Mat& in_rgb, cv::Mat& in_depth) {
  ...
  cv::Mat(depth2rgb.height, depth2rgb.width, CV_32FC1, depth2rgb.data)
      .copyTo(rgbd2);
  rgbd2.convertTo(rgbd2, CV_16UC1);
  rgbd2(cv::Rect(cv::Point(320 + 320, 61 + 240),
                 cv::Point(1600 - 320, 1021 - 240)))
      .copyTo(croppedDepth);
  rgbmat(cv::Rect(cv::Point(320 + 320, 61 + 240),
                  cv::Point(1600 - 320, 1021 - 240)))
      .copyTo(croppedBgr);
