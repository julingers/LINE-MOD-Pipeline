...
void Aruco::createArucoBoard() {
  ...
  cv::Ptr<cv::aruco::GridBoard> board =
      cv::aruco::GridBoard::create(markersX, markersY, float(markerLength),
...
void Aruco::detectBoard() {
  ...
  while (true) {
    ...
    cv::aruco::detectMarkers(image, dictionary, corners, ids,
                             ...
                             rejectedCandidates);
    cv::aruco::refineDetectedMarkers(image, board, corners, ids,
                                     rejectedCandidates);
    cv::Matx33d rotMat;
    cv::Vec3d rvec, tvec;
    glm::mat3 glmRotMat;
    ...
    if (!ids.empty()) {
      ...
      // shift cs to center of board. This was measuered by hand and should be
      // adjusted for a new board

      fromCV2GLM(cv::Mat(rotMat), &glmRotMat);
      glm::vec3 eulAng = glm::eulerAngles(glm::toQuat(glmRotMat));
      eulAng.x += CV_PI / 2;
      glmRotMat =
          glm::toMat3(glm::quat(glm::vec3(eulAng.x, eulAng.y, eulAng.z)));
      cv::putText(imageCopy,
                  glm::to_string(glm::vec3(tvec[0], tvec[1], tvec[2])),
                  cv::Point(50, 20), cv::FONT_HERSHEY_SIMPLEX, 0.5f,
                  cv::Scalar(0, 0, 255), 2.0f);
      cv::putText(imageCopy,
                  glm::to_string(
                      glm::degrees(glm::eulerAngles(glm::toQuat(glmRotMat)))),
                  cv::Point(50, 80), cv::FONT_HERSHEY_SIMPLEX, 0.5f,
                  cv::Scalar(0, 255, 255), 2.0f);
      fromGLM2CV(glmRotMat, &rotMat);
      cv::Rodrigues(rotMat, rvec);

      if (valid > 0)
        cv::aruco::drawAxis(imageCopy, cameraMatrix, distCoeffs, rvec, tvec,
                            100);
    }
    ...
    if (key == 9) {
      ...
      cv::imwrite("benchmark/depth" + std::to_string(counter) + picEnding,
                  depthImg);
