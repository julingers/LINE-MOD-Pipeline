...
HighLevelLinemodIcp::HighLevelLinemodIcp(
    uint16_t in_iteration, float in_tolerance, float in_rejectionScale,
    uint16_t in_numIterations, uint16_t in_sampleStep,
    std::vector<std::string> in_modelFiles, std::string in_modFolder)
...
HighLevelLinemodIcp::~HighLevelLinemodIcp() { delete icp; }

void HighLevelLinemodIcp::loadModels() {
  for (const auto& modelFile : modelFiles) {
    cv::Mat tempModel =
        cv::ppf_match_3d::loadPLYSimple((modelFolder + modelFile).c_str(), 1);
...
void HighLevelLinemodIcp::prepareDepthForIcp(cv::Mat& in_depth,
                                             const cv::Mat& in_camMatrix,
                                             cv::Rect& bb) {
  cv::Mat blurredDepth;
  ...
  cv::Mat sampledpatchedNaNs =
      cv::Mat(numRows, patchedNaNs.cols, patchedNaNs.type());
  ;
  int c = 0;
  ...
  cv::ppf_match_3d::computeNormalsPC3d(sampledpatchedNaNs, sceneVertices, 12,
                                       false, viewpoint);
}
...
void HighLevelLinemodIcp::registerToScene(std::vector<ObjectPose>& in_poses,
  ...
  icp->registerModelToScene(modelVertices[in_modelNumber], sceneVertices,
                            poses);
...
bool HighLevelLinemodIcp::estimateBestMatch(cv::Mat in_depthImg,
                                            std::vector<ObjectPose> in_poses,
  ...
  for (size_t i = 0; i < in_poses.size(); i++) {
    ...
    in_openglRend->renderDepthToFrontBuff(in_modelIndice, newViewMat,
                                          in_poses[i].translation);
...
void HighLevelLinemodIcp::removeIfTooFarFromMean(const cv::Mat& in_mat,
                                                 cv::Mat& in_res,
                                                 bool in_removeRows) {
  in_res.release();
...
void HighLevelLinemodIcp::depthToBinary(cv::Mat& in_gray, cv::Mat& in_binary,
...
void HighLevelLinemodIcp::updatePosition(cv::Matx44d in_mat,
                                         ObjectPose& in_objPose) {
  glm::mat4 transMat;
