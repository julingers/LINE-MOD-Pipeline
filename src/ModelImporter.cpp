...
ModelImporter::~ModelImporter() { delete importer; }

void ModelImporter::importModel(std::string const& in_modelFile,
                                Model& in_model) {
  // aiProcess_Triangulate标志位：三角化所有的面
  const aiScene* scene = importer->ReadFile(
      in_modelFile, aiProcess_Triangulate | aiProcess_OptimizeMeshes |
                        aiProcess_OptimizeGraph |
                        aiProcess_JoinIdenticalVertices |
                        aiProcess_ImproveCacheLocality);
  if (!scene || scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE, !scene->mRootNode) {
    std::cout << "ERROR:: Reading file wih Assimp "
...
// 递归处理节点和其子节点
void ModelImporter::processNode(aiNode* in_node, const aiScene* in_scene,
                                Model& in_model) {
